...
# ------------------------- #
# Probability to CR adapter #
# ------------------------- #

class ProbabilityToCRAdapter(object):
    def __init__(self, file_name):
        ...
        try:
            with ZipFile(file_name, 'r') as z:
                with z.open("thresholds", 'r') as f:
                    for line in map(str.strip, f):
                        if line == '': continue
                        cols = line.split('\t')
                        if cols[0] == 'thresholds':
                            self._thresholds = list(map(float, cols[1:]))
                        elif cols[0] == 'crs':
                            self._crs = list(map(float, cols[1:]))
        except Exception:
            pass  # ignore, there is a fallback in __call__ to NOP

    def __call__(self, probability):
        if self._thresholds is None:
            return probability  # fall back for old models
        else:
            # ASSUMPTION: thresholds are sorted in descending order
            # find the first index in the thresholds probability > t, if none, otherwise use last CR
            cr_index = next(filter(lambda i, t: probability >= t, enumerate(self._thresholds)), (len(self._thresholds),))[0]
            return self._crs[cr_index]
...
class BayesModel(object):
    ...
    def _indices_to_combination_indices(self, individual_indices):
        # This way is twice as fast as this: [(x, y) for x in individual_indices for y in individual_indices if x <= y]
        return zip(individual_indices, individual_indices) + list(combinations(individual_indices, 2))
    ...
    def predict_from_indices_combinations(self, combinations_list):
        ...
        denum = logsumexp(ljls)
        log_prob1 = ljls[1] - denum
        log_prob0 = ljls[0] - denum
...
class CombinedModel(object):
    def __init__(self, hist_model, recent_model, alpha=1.0):
        """
    ...
    def predict(self, feature_list):
        ...
        return hp * self._alpha + rp * (1.0 - self._alpha), list(set(hf + rf))
...
def load(model_files):
    ...
    cr_adapter = ProbabilityToCRAdapter(model_zip_file)
    ...
    if recent_model.exists():
        alpha = 1.0
        try:
            with ZipFile(model_zip_file, 'r') as z:
                with z.open("interpolation-params", 'r') as f:
                    for line in map(str.strip, f):
                        if line == '': continue
                        cols = line.split('\t')
                        if cols[0] == 'alpha':
            ...
            pass  # ignore, just fallback to alpha 1.0

        if alpha < 1.0:
            full_model = CombinedModel(historical_model, recent_model, alpha)

    model_id = str(uuid.uuid4())
    id2model[model_id] = (full_model, cr_adapter)
    return model_id


def free(model_id):
    if model_id in id2model:
        del id2model[model_id]
    return "free"


def predict(model_id, all_agents_props_list, bid_request_props, num_single_agent_props):
    model, cr_adapter = id2model[model_id]
    num_agents = len(all_agents_props_list) / num_single_agent_props if num_single_agent_props > 0 else 1
    predictions = []
    attributes = []
    ...
    for agent_index in range(num_agents):
        ...
        predictions.append(cr_adapter(probability))
    ...
    return json.dumps(
        {"request": [{"name": "data_center", "field": "dc_log"},
                     ...
                     {"name": "TOD", "field": "device.ext.timezone_offset", "other_field": "timezone",
                      "type": 2,
                      "function": "var dynamicFeaturesFunc=function(keyWord,timezone_offset){var date=new Date();var dayTime=date.getHours();var tod=((dayTime + 24 + timezone_offset) % 24).toString();return keyWord+tod;}"}
                     ],
